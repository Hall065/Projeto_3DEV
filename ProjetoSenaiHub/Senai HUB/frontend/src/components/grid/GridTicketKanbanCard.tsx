    ...
    >
      ...
      <div className="border-b border-hub-border/40 bg-gradient-to-r from-hub-navy/5 to-transparent px-3 py-2.5">
        ...
        <div className="mt-2 flex flex-wrap gap-1" onPointerDown={(e) => e.stopPropagation()}>
          ...
          <button type="button" className="rounded p-1.5 hover:bg-white/50" onClick={onAssign} aria-label="Atribuir">
      ...
      <div className="space-y-2 px-3 py-2.5 text-sm">
        ...
        <p className="text-[11px] text-hub-text-muted">{formatDateTime(ticket.opened_at)}</p>

        {ticket.assignee ? (

          <p className="truncate text-xs text-hub-text-muted">

            <span className="font-medium text-hub-text">Técnico:</span> {ticket.assignee}
        ...
        {locked ? (
          <p className="text-[11px] font-medium text-amber-700">Aguardando conclusão da tarefa vinculada</p>
        ) : null}
