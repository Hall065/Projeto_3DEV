...
function StatusBadge({ status }: { status: CampusTicketMarkerStatus }) {
  return (
    ...
    >
      {CAMPUS_TICKET_STATUS_LABELS[status]}
    </span>
  )
}
      ...
      {selected ? (
        <div className="glass-panel-solid pointer-events-auto rounded-xl border border-hub-border/60 p-3 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-wide text-hub-red">
            {CAMPUS_TICKET_KIND_LABELS[selected.kind]} selecionado
          </p>
          ...
          {selected.room && <p className="mt-2 text-xs text-hub-text-muted">Sala: {selected.room}</p>}
          {selected.assignee && (
            <p className="text-xs text-hub-text-muted">Responsavel: {selected.assignee}</p>
          )}
          <p className="text-xs text-hub-text-muted capitalize">Prioridade: {selected.priority}</p>
        </div>
      ) : selectedBlockId && blockTickets.length > 0 ? (
        <div className="glass-panel-solid pointer-events-auto rounded-xl border border-hub-border/60 p-3 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-wide text-hub-red">{blockName}</p>
          <p className="mb-2 text-[11px] text-hub-text-muted">{blockTickets.length} atendimento(s) no bloco</p>
            ...
            {blockTickets.map((item) => (
              <li key={item.id} className="flex items-start gap-2 text-xs">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium text-hub-navy">{item.code} · {item.title}</p>
                  <p className="truncate text-hub-text-muted">
                    {item.room ? `Sala ${item.room}` : item.statusLabel}
                  </p>
                </div>
        ...
        <div className="glass-panel-solid pointer-events-auto rounded-xl border border-hub-border/60 px-3 py-2 text-xs text-hub-text-muted shadow-lg">
          Nenhum chamado ou tarefa em {blockName} no momento.
        </div>
      ) : (
        <div className="glass-panel-solid pointer-events-auto rounded-xl border border-hub-border/60 p-3 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-wide text-hub-red">Chamados no campus</p>
          <p className="mt-1 text-[11px] text-hub-text-muted">Clique em um marcador ou bloco para detalhes</p>
          <dl className="mt-2 grid grid-cols-3 gap-2 text-center">
            <div>
              <dt className="text-[10px] text-hub-text-muted">Abertos</dt>
              <dd className="text-base font-bold text-red-600">{statusTotals.open}</dd>
            </div>
            <div>
              <dt className="text-[10px] text-hub-text-muted">Em andamento</dt>
              <dd className="text-base font-bold text-blue-600">{statusTotals.in_progress}</dd>
            </div>
            <div>
              <dt className="text-[10px] text-hub-text-muted">Concluidos</dt>
          ...
          <p className="mt-2 text-center text-[10px] text-hub-text-muted">
            {kindTotals.ticket} chamado(s) · {kindTotals.task} tarefa(s)
          </p>
        </div>
      )}
    </div>
